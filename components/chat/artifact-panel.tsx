...
import { X, Copy, Download, ChevronLeft, ChevronRight, Check } from "lucide-react";
...
const MarkdownView = dynamic(() => import("./artifacts/markdown-view"), { ssr: false });
const SpreadsheetView = dynamic(() => import("./artifacts/spreadsheet-view"), { ssr: false });
...
export function ArtifactPanel({ 
  ...
  const handleDownload = () => {
    ...
    const extension = artifact.type === "markdown" ? "md" : 
                     artifact.type === "html" ? "html" :
                     artifact.type === "mermaid" ? "mmd" : "txt";
    ...
    <div className="w-full md:w-[60%] lg:w-[55%] xl:w-[50%] h-full border-l bg-card flex flex-col shadow-xl md:shadow-none absolute right-0 top-0 z-50 md:relative overflow-hidden transition-all duration-300 animate-in slide-in-from-right">
      <div className="flex items-center justify-between p-3 border-b bg-muted/30 shrink-0">
        <div className="flex items-center gap-3 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            <h3 className="font-semibold text-sm truncate">{artifact.title || "Artifact"}</h3>
            <span className="text-[10px] text-muted-foreground uppercase leading-none">{artifact.type}</span>
          </div>
        </div>
      ...
      <div className="flex-1 relative overflow-hidden bg-muted/5">
        <Tabs value={artifact.type} className="w-full h-full flex flex-col">
          ...
          <TabsContent value="markdown" className="w-full h-full m-0 border-none p-0 outline-none">
            <MarkdownView id={`${artifact.messageId}-${currentIndex}`} content={artifact.content} onUpdate={onUpdate} />
          </TabsContent>
          
          <TabsContent value="spreadsheet" className="w-full h-full m-0 border-none p-0 outline-none">
            <SpreadsheetView title={artifact.title} content={artifact.content} />
          </TabsContent>
          
          <TabsContent value="html" className="w-full h-full m-0 border-none p-0 outline-none">
            <HtmlView content={artifact.content} />
          </TabsContent>
          
          <TabsContent value="mermaid" className="w-full h-full m-0 border-none p-6 outline-none overflow-auto custom-scrollbar">
