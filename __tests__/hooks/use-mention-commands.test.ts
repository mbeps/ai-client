...
vi.mock("@/lib/actions/chats/delete-message", () => ({ deleteMessage: vi.fn() }));
vi.mock("@/lib/actions/chats/update-current-leaf", () => ({ updateCurrentLeaf: vi.fn() }));
vi.mock("@/lib/actions/chats/update-message-metadata", () => ({ updateMessageMetadata: vi.fn() }));
vi.mock("@/lib/actions/projects/list-projects", () => ({ listProjects: vi.fn() }));
vi.mock("@/lib/actions/projects/create-project", () => ({ createProject: vi.fn() }));
vi.mock("@/lib/actions/projects/delete-project", () => ({ deleteProject: vi.fn() }));
vi.mock("@/lib/actions/projects/rename-project", () => ({ renameProject: vi.fn() }));
vi.mock("@/lib/actions/projects/update-project", () => ({ updateProject: vi.fn() }));
vi.mock("@/lib/actions/projects/toggle-project-pin", () => ({ toggleProjectPin: vi.fn() }));
vi.mock("@/lib/actions/assistants/list-assistants", () => ({ listAssistants: vi.fn() }));
vi.mock("@/lib/actions/assistants/create-assistant", () => ({ createAssistant: vi.fn() }));
vi.mock("@/lib/actions/assistants/delete-assistant", () => ({ deleteAssistant: vi.fn() }));
vi.mock("@/lib/actions/assistants/rename-assistant", () => ({ renameAssistant: vi.fn() }));
vi.mock("@/lib/actions/assistants/update-assistant", () => ({ updateAssistant: vi.fn() }));
vi.mock("@/lib/actions/prompts/list-prompts", () => ({ listPrompts: vi.fn() }));
vi.mock("@/lib/actions/prompts/create-prompt", () => ({ createPrompt: vi.fn() }));
vi.mock("@/lib/actions/prompts/delete-prompt", () => ({ deletePrompt: vi.fn() }));
vi.mock("@/lib/actions/prompts/update-prompt", () => ({ updatePrompt: vi.fn() }));
...
vi.mock("@/lib/actions/projects/toggle-pin-project", () => ({ togglePinProject: vi.fn() }));
vi.mock("@/lib/actions/knowledgebases/rename-knowledgebase", () => ({ renameKnowledgebase: vi.fn() }));
...
const SAMPLE_PROMPTS = [
  { id: "p1", title: "Summarise", shortcut: "sum", content: "Summarise the following:", updatedAt: new Date() },
  { id: "p2", title: "Translate", shortcut: "trans", content: "Translate to English:", updatedAt: new Date() },
];

const SAMPLE_ASSISTANTS = [
  { id: "a1", name: "Code Reviewer", description: "Reviews code", prompt: "You are a code reviewer", tools: [], knowledgebases: [], updatedAt: new Date() },
  { id: "a2", name: "Translator", description: "Translates text", prompt: "You are a translator", tools: [], knowledgebases: [], updatedAt: new Date() },
];

function makeInputEvent(value: string, selectionStart: number) {
  return { target: { value, selectionStart } } as React.ChangeEvent<HTMLTextAreaElement>;
}
...
describe("useMentionCommands", () => {
  ...
  beforeEach(() => {
    ...
    useAppStore.setState({ prompts: SAMPLE_PROMPTS, assistants: SAMPLE_ASSISTANTS });
    vi.clearAllMocks();
  });
  ...
  describe("initial state", () => {
    it("starts with openTrigger=null", () => {
      ...
      const { result } = renderHook(() => useMentionCommands("", setInput, textareaRef));
  ...
  describe("mention command detection (@)", () => {
    ...
    it("does not open if activeChatAssistantId is set", () => {
      const setInput = vi.fn();
      const { result } = renderHook(() => useMentionCommands("", setInput, textareaRef, "some-id"));
    ...
    it("shows all assistants when query is empty (@)", () => {
      ...
      expect(result.current.filteredItems).toHaveLength(SAMPLE_ASSISTANTS.length);
    });
    ...
    it("filters assistants by name match", () => {
      ...
      expect(result.current.filteredItems).toHaveLength(1);
      expect((result.current.filteredItems[0] as any).name).toBe("Code Reviewer");
    });
  });
  ...
  describe("handleSelect", () => {
    it("removes the / trigger and sets selectedPrompt", () => {
      const setInput = vi.fn();
      const { result } = renderHook(() => useMentionCommands("/sum", setInput, textareaRef));
    ...
    it("removes the @ trigger and sets selectedAssistant", () => {
      const setInput = vi.fn();
      const { result } = renderHook(() => useMentionCommands("@code", setInput, textareaRef));
